...
data_to_drop = ['case:AMOUNT_REQ', 'Next Event', 'case:REG_DATE', 'concept:name', 'case:concept:name', 'time:timestamp', 'position', 'minute',
...
print("Any null values in the training data?: ",X_train.isnull().values.any())
print("\nAny null values in the testing data?: ",y_train.isnull().values.any())
...
X_test = X_test[['org:resource', 'lifecycle:transition', 'A_SUBMITTED', 'A_PARTLYSUBMITTED', 'A_PREACCEPTED',
...
#Add the findings to the visualization Dataframe
visualization['Next time'] = y_test.to_numpy()
visualization['Predicted time'] = y_pred

#END of the implemenation of the Linear Regression
print(df_encoded_test.corr())
